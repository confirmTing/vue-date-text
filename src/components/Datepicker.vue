      ...
      <span class="vdp-datepicker__clear-button" :class="{'input-group-addon' : bootstrapStyling}" v-if="clearButton && selectedDate" @click="clearDate()"><i :class="clearButtonIcon"><span v-if="calendarButtonIcon.length === 0">&times;</span></i></span>
        ...
        <div :class="[calendarClass, 'vdp-datepicker__calendar']" v-show="showDayView" v-bind:style="calendarStyle">
            ...
            --><span class="cell day"
                v-for="day in days"
                track-by="timestamp"
                v-bind:class="{ 'selected':day.isSelected, 'disabled':day.isDisabled, 'highlighted': day.isHighlighted, 'today': day.isToday, 'day-text':day.text}"
                @click="selectDate(day)">{{ day.date }} <span v-if="day.text"><br>{{day.text}}</span></span>
        </div>
...
export default {
  ...
  methods: {
    ...
    setDate (timestamp, text) {
      const date = new Date(timestamp)
      this.selectedDate = new Date(date)
      this.setPageDate(date)
      let _dStr = DateUtils.formatDate(date, this.format, this.translation)
      this.$emit('selected', {
        date: _dStr,
        text
      })
      this.$emit('input', _dStr)
    },
    ...
    selectDate (day) {
      ...
      this.setDate(day.timestamp, day.text)
      if (this.isInline) {
    ...
    isDisabledDate (date) {
      ...
      if (!this.dateText) {
        ...
        if (this.dateText.map(d => d.date.toDateString ? d.date.toDateString() : new Date(d.date).toDateString()).indexOf(date.toDateString()) === -1) {
          disabled = true
        }
      }
      return disabled
    },
    isTextDate (date) { // 放入文字到日期内
      if (!this.dateText) {
        return false
      }
      let text = false
      this.dateText.forEach((d) => {
        let _d = d.date
        let _dStr = d.date.toDateString ? d.date.toDateString() : new Date(_d).toDateString()
        if (date.toDateString() === _dStr) {
          text = d.text
          return true
        }
      })
